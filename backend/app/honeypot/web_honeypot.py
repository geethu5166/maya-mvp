"""
Web Honeypot - Fake banking portal for credential harvesting detection
"""
...
def create_web_honeypot(app: Optional[Flask] = None) -> Flask:
    """Create and configure the web honeypot Flask app"""
    ...
    def log_attack(data: dict):
        """Log attack to event pipeline"""
        try:
            data['timestamp'] = datetime.datetime.now().isoformat()
            logger.warning(
                f"[WEB HONEYPOT] {data['type']} from {data['attacker_ip']} | "
                f"Path: {data.get('path', 'N/A')}"
            )
            event = event_pipeline.publish(data)
            logger.info(f"[WEB HONEYPOT] Event {event.get('event_id')} published")
        except Exception as e:
            logger.error(f"Failed to publish web attack event: {e}")
...
<a href="/">Back to Login</a>
    ...
    def login():
        """Login handler - harvest credentials"""
    ...
    def traps():
        """Traps for common scanner paths"""
        log_attack({
            "type": "WEB_SCAN",
            "attacker_ip": request.remote_addr,
            "honeypot": "WEB",
...
if __name__ == '__main__':
    ...
    logger.info("[WEB HONEYPOT] Starting on http://0.0.0.0:5001")
